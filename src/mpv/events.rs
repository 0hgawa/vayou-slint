...
/// Persist the playhead for the file being played, so reopening resumes there.
///
/// Called on END_FILE, on SHUTDOWN and every 30s from the loop below — and from
/// `main` on quit, which is the one exit none of those cover.
pub(crate) fn save_position(mpv: &MpvPlayer, app: &AppState) {
