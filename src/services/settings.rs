...
/// The UI's own font family.
///
/// Windows names Segoe UI directly. On Linux this follows the desktop rather
/// than a face we picked: Cantarell is the GNOME UI font, and where it is absent
/// fontconfig substitutes the session's sans-serif, which is the right answer on
/// every other desktop too.
#[cfg(windows)]
pub const UI_FONT: &str = "Segoe UI";
#[cfg(unix)]
pub const UI_FONT: &str = "Cantarell";

/// Families offered in the subtitle font picker.
///
/// Per-platform because the Windows list is Microsoft's own — Tahoma, Calibri,
/// Consolas — and none of it exists on a stock Linux desktop. libass would
/// substitute silently, so subtitles would still render, but the picker would be
/// listing fonts the user does not have and every choice would look the same.
/// The Linux list is what a distribution's base font packages actually install.
#[cfg(windows)]
pub const SUBTITLE_FONTS: &[&str] =
    &["Segoe UI", "Arial", "Tahoma", "Verdana", "Trebuchet MS", "Calibri", "Consolas", "Impact", "Georgia"];
#[cfg(unix)]
pub const SUBTITLE_FONTS: &[&str] =
    &["DejaVu Sans", "Liberation Sans", "Cantarell", "DejaVu Serif", "Liberation Serif", "DejaVu Sans Mono", "Noto Sans Mono"];

/// The subtitle face used until the user picks another. DejaVu Sans is
/// fontconfig's own sans-serif default and ships with essentially every
/// distribution, so it is the one name safe to write into a fresh config.
#[cfg(windows)]
pub const DEFAULT_SUBTITLE_FONT: &str = "Segoe UI";
#[cfg(unix)]
pub const DEFAULT_SUBTITLE_FONT: &str = "DejaVu Sans";
...
impl From<&crate::services::tracks::SubStyle> for SubtitleStyleSettings {
    fn from(s: &crate::services::tracks::SubStyle) -> Self {
        Self {
            ...
            position: s.position, bold: s.bold, shadow: s.shadow,
        }
    }
}
...
impl Default for SubtitleStyleSettings {
    fn default() -> Self {
        Self {
            font: DEFAULT_SUBTITLE_FONT.into(),
            // mpv's own default, which is the only documented recommendation
            // there is for this number — it is measured in scaled pixels against
            // a 720px-tall window, so it tracks the window rather than the
            // display. The 55 this carried for both builds was 45% above it with
            // nothing behind the choice, and read as oversized.
            size: 38,
            color: "#ffffff".into(),
