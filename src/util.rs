...
/// Whether two ISO-639 codes name the same language, across the 639-1/639-2
/// split ("pt" and "por") and across regions ("pt-BR" and "pt").
///
/// An empty code matches nothing, not even another empty one: an untagged track
/// says nothing about what is in it.
pub fn same_language(a: &str, b: &str) -> bool {
    fn base(code: &str) -> String {
        let name = lang_name(code);
        name.split_once(" (").map_or(name.as_str(), |(base, _)| base).to_string()
    }
...
#[cfg(test)]
mod tests {
    use super::{lang_name, same_language, track_label};

    #[test]
    fn same_language_crosses_the_639_split_and_regions() {
        assert!(same_language("por", "pt")); // 639-2 vs 639-1
        assert!(same_language("pt-BR", "pt")); // region ignored
        assert!(same_language("pob", "por")); // both spellings of Brazilian
        assert!(same_language("xx", "xx")); // unknown, but the same code
        assert!(!same_language("eng", "pt"));
        assert!(!same_language("", "")); // untagged tracks match nothing
        assert!(!same_language("pt", ""));
    }
