...
/// Where this grab is going, with the folder made and ready for it.
fn screenshot_path() -> Result<std::path::PathBuf, String> {
...
///
/// The frame is read back from the GL framebuffer rather than asked of mpv:
/// `vo=libmpv` is not a video output `screenshot-to-file` can take an image
/// from, and it answers every mode of that command — `subtitles`, `video` and
/// `window` alike — with a bare "error running command". Which is what this did
/// before: log a failure and leave the folder empty.
pub(crate) fn screenshot_to_toast(ui: &MainWindow) {
    let toast = |ui: &MainWindow, r: Result<(), String>| match r {
        Ok(()) => ui.set_toast("Screenshot saved".into()),
        Err(e) => {
    ...
    let path = match screenshot_path() {
        Ok(p) => p,
        Err(e) => return toast(ui, Err(e)),
    };
    let ui_w = ui.as_weak();
    crate::video_render::request_screenshot(path, move |r| {
        if let Some(ui) = ui_w.upgrade() {
            toast(&ui, r);
        }
    });
    // Paused, nothing else would bring the notifier round and the grab would
    // never happen.
...
pub(crate) fn wire(ui: &MainWindow, mpv_state: &Arc<MpvState>, app_state: &Arc<AppState>) {
    ...
    ui.on_seek_fraction({
        ...
        move |frac, exact| {
            ...
            if let Ok(m) = mpv.get() { let _ = PlaybackService::seek_absolute(m, t, exact); }
        }
    });
    ui.on_adjust_volume({
    ...
    ui.on_screenshot({
        let ui_w = ui.as_weak();
        move || { if let Some(ui) = ui_w.upgrade() { screenshot_to_toast(&ui); } }
    });
    ui.on_seek_chapter({
        let mpv = mpv_state.clone();
