...
pub(crate) fn wire(ui: &MainWindow, mpv_state: &Arc<MpvState>) {
    ...
    ui.on_playlist_add_files({
        let (ui_w, mpv) = (ui.as_weak(), mpv_state.clone());
        move || {
            let (weak, mpv) = (ui_w.clone(), mpv.clone());
            crate::bridge::pick_async(
                || rfd::FileDialog::new().add_filter("Media", crate::services::playlist::MEDIA_EXTENSIONS).add_filter("All", &["*"]).pick_files(),
                move |files: Vec<std::path::PathBuf>| {
                    let Ok(m) = mpv.get() else { return };
                    for f in &files { let _ = PlaylistService::add(m, &f.to_string_lossy()); }
                    if let Some(ui) = weak.upgrade() { refresh_playlist(&ui, m); }
                },
            );
        }
    });
        ...
        move || {
            ...
            crate::bridge::pick_async(
                ...
                move |dir: std::path::PathBuf| {
                    let Ok(m) = mpv.get() else { return };
                    let _ = PlaylistService::add(m, &dir.to_string_lossy());
                    if let Some(ui) = weak.upgrade() { refresh_playlist(&ui, m); }
                },
            );
        }
