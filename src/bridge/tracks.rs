...
pub(crate) fn wire(ui: &MainWindow, mpv_state: &Arc<MpvState>, app_state: &Arc<AppState>) {
    ui.on_select_sub({
        ...
        move |id| {
            ...
            // skip "Disable" and our own translation output. An image-based
            // track goes through: the job falls back to a text track in the
            // same file, or says on the banner why it cannot.
            // A job already running is left to finish: picking a source track is
            // not the same as changing one's mind about the target language, and
            // restarting here would throw away an extraction under way.
            if id >= 0 && ui.get_translate_lang().as_str() != "off" && !ui.get_translating() {
                if let Ok(m) = mpv.get() {
