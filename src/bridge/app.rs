...
use crate::bridge::subtitle::{
    apply_sub_style, reset_search_for_current_file, reset_translation_for_new_file, start_translation,
};
use crate::error::{LogErr, MpvError};
use crate::mpv::events::{start_event_loop, EventSink, PlayerEvent};
use crate::mpv::ffi::INSTALL_STEPS;
    ...
    std::thread::spawn(move || {
        let mpv = match MpvPlayer::new() {
            ...
            Err(e) => {
                ...
                // Stderr is where this used to end, and nobody launching from a
                // desktop menu ever sees stderr: they get a window that opens,
                // accepts a file and plays nothing. Put it where they are.
                //
                // A missing library is the one case with an answer worth
                // printing; the rest are internal enough that the error text
                // itself is the most honest thing to show.
                let detail = match &e {
                    MpvError::LibraryLoad(_) => format!("libmpv could not be loaded.\n\n{INSTALL_STEPS}"),
                    other => other.to_string(),
                };
                let _ = weak.upgrade_in_event_loop(move |ui| ui.set_mpv_error(detail.into()));
                return;
            }
        };
        ...
        // Observe volume only AFTER applying the saved level, so the observer's
        // initial value is that level — not mpv's 100 default, which would
        // otherwise echo to the UI and briefly show 100 on open.
    ...
    Arc::new(move |ev: PlayerEvent| {
        ...
        let _ = slint::invoke_from_event_loop(move || {
            ...
            match ev {
                ...
                PlayerEvent::MediaTitle(t) => {
                    ui.set_media_title(t.into());
                    // The title changing is the film changing, which is what the
                    // OpenSubtitles page has to follow — otherwise the previous
                    // film's results stay on screen under the new film's name.
                    // Doing it here rather than on FILE_LOADED is deliberate:
                    // the new title may not have arrived by then, and this is
                    // the moment it has.
                    reset_search_for_current_file(&ui);
                }
                PlayerEvent::FileLoaded => {
                    ui.set_has_file(true);
                    reset_translation_for_new_file(&ui);
                    ...
                    let target = app_state.with(|s, _| s.translate_lang.clone()).unwrap_or_else(|_| "off".into());
                    if target != "off" {
                        ...
                        slint::Timer::single_shot(Duration::from_millis(800), move || {
                            ...
                            // A release that already carries the target language gains
                            // nothing from a machine translation of another track into
                            // it, and skipping it saves the whole run: an ffmpeg
                            // extraction plus one request per chunk of dialogue, on
                            // every file opened.
                            if tracks.iter().any(|t| t.track_type == "sub" && util::same_language(&t.lang, &target)) {
                                return;
                            }
                            if tracks.iter().any(|t| t.track_type == "sub" && t.selected) {
...
/// Open the native file dialog OFF the UI thread. A blocking dialog on the
/// event-loop thread stalls the render loop — the video image freezes (audio
/// keeps going) and folder navigation fights mpv for the CPU. Running it on a
/// blocking task keeps playback smooth; the chosen file is loaded back on the
/// UI thread.
pub(crate) fn open_via_dialog(ui: &MainWindow, mpv_state: &Arc<MpvState>, app_state: &Arc<AppState>) {
    let (weak, mpv, app) = (ui.as_weak(), mpv_state.clone(), app_state.clone());
    crate::bridge::pick_async(
        || rfd::FileDialog::new()
        ...
        move |p: std::path::PathBuf| {
            let Some(ui) = weak.upgrade() else { return };
            ui.set_has_file(true);
            open_file(&p.to_string_lossy(), &mpv, &app);
        },
    );
}
