...
pub(crate) fn wire(ui: &MainWindow, mpv_state: &Arc<MpvState>, app_state: &Arc<AppState>) {
    ...
    // Both sliders emit on every pointer move, so the write is debounced.
    ui.on_set_def_volume({
        let app = app_state.clone();
        move |v| { let _ = app.with(|s, _| s.volume = f64::from(v)); persist::save_debounced(&app); }
    });
    ui.on_set_def_speed({
        let app = app_state.clone();
        move |v| { let _ = app.with(|s, _| s.speed = f64::from(v)); persist::save_debounced(&app); }
    });
    ...
    ui.on_relaunch_app({
        ...
        move || {
            ...
            // Same exit as the close button: the replacement is already starting,
            // and the old window hanging about while this one dies looks like two
            // copies of the app.
            if let Some(ui) = ui_w.upgrade() { crate::bridge::window::close_app(&ui); }
        }
    });
