...
/// The exe icon and version metadata — both Windows resource files.
///
/// `cfg(windows)` in a build script tests the HOST, which is also what gates the
/// `embed-resource` build dependency in Cargo.toml. The two therefore agree on
/// any native build: on a Linux host neither the crate nor this code exists.
#[cfg(windows)]
fn embed_windows_resources() {
    // Embed the brand icon as the exe/taskbar icon — compile-time only.
    ...
    // Stamp the exe (debug AND release) with version metadata, generated from
    // the crate version so the file's properties (File/Product version) are
...
fn embed_version_resource() {
    ...
    let rc = format!(
        "1 VERSIONINFO\n\
         ...
         \x20     VALUE \"FileDescription\", \"Vayou - native video player\"\n\
