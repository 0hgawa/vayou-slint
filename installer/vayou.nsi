...
; Source artefacts (the built release), relative to this file. They used to be
; absolute and named one machine's D: drive, so the installer could only ever be
; built there.
;
; Plain relative paths, not ${__FILEDIR__}: makensis already resolves these
; against the .nsi's own directory, so prefixing that variable doubles the
; segment when the script is passed as a relative path -- installer\installer\..
; -- and the build dies on the first icon. An absolute invocation hides it,
; which is exactly how it reached CI.
;
; Each stays overridable with /D for a build that stages its artefacts elsewhere.
!ifndef HERE
  !define HERE   "."
!endif
!ifndef SRC
