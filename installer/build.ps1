...
#   -Key   path to the rsign/minisign secret key (default: ../../.keys/vayou.key,
#          i.e. a .keys folder beside the repo; override for anywhere else)
...
# Derived from this script's own location rather than written out: the paths
# used to be absolute and named one machine's D: drive, so the script only ever
# worked there - and stopped working at all once that install was gone.
$root = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$exe  = Join-Path $root "target\release\vayou.exe"
if (-not $Key) { $Key = Join-Path (Split-Path $root -Parent) ".keys\vayou.key" }
...
# 3. Write this platform's entry into latest.json (reusing $version read above).
#
# The feed is shared: it carries one entry per platform, and each release script
# writes only its own. Rebuilding the object from scratch would drop the Linux
# entry build-linux.sh put there, and every installed Linux copy would then read
# the release as having nothing for it. The two builds run on different machines,
# so the manifest is carried between them and each completes the other's half.
$latest = Join-Path $PSScriptRoot "latest.json"
$platforms = [ordered]@{}
if (Test-Path $latest) {
    $existing = [System.IO.File]::ReadAllText($latest) | ConvertFrom-Json
...
Write-Host "Manifest platforms: $($platforms.Keys -join ', ')"
